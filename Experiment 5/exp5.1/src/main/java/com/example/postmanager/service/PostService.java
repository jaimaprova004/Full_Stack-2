...
public class PostService {
    ...
    public Post create(PostDTO dto) {
        log.info("Creating new post for author={} title={}", dto.getAuthor(), dto.getTitle());
        Post p = new Post(dto.getTitle(), dto.getContent(), dto.getAuthor());
        return repo.save(p);
    }

    public List<Post> listAll() {
        log.debug("Listing all posts");
        return repo.findAll();
    }
    ...
    public Post update(Long id, PostDTO dto) {
        Post existing = repo.findById(id)
                .orElseThrow(() -> {
                    log.warn("Update failed because post id={} was not found", id);
                    return new ResourceNotFoundException("Post", id);
                });
