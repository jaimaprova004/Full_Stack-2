...
public class PostController {
    ...
    public ResponseEntity<ApiResponse<Post>> create(@Valid @RequestBody PostDTO dto, HttpServletRequest request) {
        ...
        return ResponseEntity.status(201)
                .body(ApiResponse.created(created, request.getRequestURI(), request.getHeader("X-Request-Id")));
    }
    ...
    public ResponseEntity<ApiResponse<Post>> get(@PathVariable Long id, HttpServletRequest request) {
        ...
        return ResponseEntity.ok(ApiResponse.ok(post, request.getRequestURI(), request.getHeader("X-Request-Id")));
    }

    @PutMapping("/{id}")
    public ResponseEntity<ApiResponse<Post>> update(@PathVariable Long id,
